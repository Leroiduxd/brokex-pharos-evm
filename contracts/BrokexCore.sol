...
contract BrokexCore {
    ...
    struct AssetConfig {
        ...
        uint256 maxProofAge;        // seconds — max age for Supra proof AND KMS proof
        ...
        uint256 guaranteedSLFeeBps; // e.g. 1_000 = 0.1% fee on OI for guaranteed SL (PRECISION-scaled)
        bool listed;
        bool frozen;
    }
    ...
    struct Trade {
        ...
        bool    guaranteedSL;   // true if trader paid premium fee for guaranteed target stop loss execution
    }

    // Risk proof signed off-chain by AWS KMS.
    // Spread is the SOLE source of spread — no on-chain baseSpread.
    // Signature covers ALL six fields: supraId, maxOILong, maxOIShort, spreadLong, spreadShort, timestamp.
    struct RiskProof {
        uint256 supraId;
        ...
        bytes   sig;          // ECDSA over keccak256(abi.encode(supraId,maxOILong,maxOIShort,spreadLong,spreadShort,timestamp))
    }
    ...
    event AssetListed(uint256 indexed supraId);
    event AssetUpdated(uint256 indexed supraId);
    event AssetFrozen(uint256 indexed supraId);
    event AssetUnfrozen(uint256 indexed supraId);
    event AssetDelisted(uint256 indexed supraId);

    function listAsset(uint256 supraId, AssetConfig calldata cfg) external onlyOwner {
        if (assets[supraId].listed) revert BadParameter(); // Already listed
        _validateConfig(cfg);
        AssetConfig memory newCfg = cfg;
        newCfg.listed = true;
        newCfg.frozen = false;
        assets[supraId] = newCfg;
        emit AssetListed(supraId);
    }

    function updateAsset(uint256 supraId, AssetConfig calldata cfg) external onlyOwner {
        if (!assets[supraId].listed) revert BadParameter(); // Not listed
        _validateConfig(cfg);
        AssetConfig memory newCfg = cfg;
        // profitCap, borrowRateHourly, lockedCapitalBps, liqThresholdBps, and guaranteedSLFeeBps are immutable once listed
        newCfg.profitCap = assets[supraId].profitCap;
        newCfg.borrowRateHourly = assets[supraId].borrowRateHourly;
        newCfg.lockedCapitalBps = assets[supraId].lockedCapitalBps;
        newCfg.liqThresholdBps = assets[supraId].liqThresholdBps;
        newCfg.guaranteedSLFeeBps = assets[supraId].guaranteedSLFeeBps;
        newCfg.listed = true;
        newCfg.frozen = assets[supraId].frozen;
        assets[supraId] = newCfg;
        emit AssetUpdated(supraId);
    }

    function freezeAsset(uint256 supraId) external onlyOwner {
        if (!assets[supraId].listed) revert BadParameter();
        assets[supraId].frozen = true;
        emit AssetFrozen(supraId);
    }

    function unfreezeAsset(uint256 supraId) external onlyOwner {
        if (!assets[supraId].listed) revert BadParameter();
        assets[supraId].frozen = false;
        emit AssetUnfrozen(supraId);
    }

    function delistAsset(uint256 supraId) external onlyOwner {
        if (!assets[supraId].listed) revert BadParameter();
        if (openInterestLong[supraId] != 0 || openInterestShort[supraId] != 0) revert BadParameter();
        assets[supraId].listed = false;
        emit AssetDelisted(supraId);
    }
    ...
    function openMarketPosition(
        ...
        AssetConfig storage cfg = assets[supraId];
        if (!cfg.listed) revert BadParameter();
        if (cfg.frozen) revert BadParameter();
        if (riskProof.supraId != supraId) revert BadParameter();
        if (guaranteedSL && slPrice == 0) revert BadSLTP();
        ...
        (uint256 margin, uint256 oi) = _pullFundsAndCommission(supraId, collateral, leverage);

        uint256 oraclePrice = _getPrice(supraId, oracleProof);
        ...
        _applyRisk(supraId, direction, oi, riskProof);
    ...
    function modifyStops(uint256 tradeId, uint256 newSL, uint256 newTP, bool guaranteedSL) external {
        Trade storage t = trades[tradeId];
        if (t.trader != msg.sender) revert NotTrader();
        if (t.state != STATE_OPEN && t.state != STATE_ORDER) revert InvalidState();
        if (guaranteedSL && newSL == 0) revert BadSLTP();

        if (newSL != 0 || newTP != 0) {
            uint256 refPrice = t.state == STATE_OPEN ? t.openPrice   : t.targetPrice;
            uint256 liqP     = t.state == STATE_OPEN
                ? _liqPrice(t.openPrice, t.leverage, t.direction, assets[t.supraId].liqThresholdBps)
                : _liqPrice(t.targetPrice, t.leverage, t.direction, assets[t.supraId].liqThresholdBps);
            _validateSLTP(t.direction, refPrice, liqP, newSL, newTP);
        }

        t.stopLoss     = newSL;
        t.takeProfit   = newTP;
        t.guaranteedSL = t.state == STATE_OPEN ? false : guaranteedSL;
        emit TradeEvent(tradeId);
    }
    ...
    function closePositionMarket(
        ...
        if (t.supraId != supraId) revert BadParameter();
        if (riskProof.supraId != supraId) revert BadParameter();
    ...
    ///
    /// @dev     CRITICAL: The `oracleProof` bytes payload must contain verified price data for
    ///          ALL distinct `supraId` assets belonging to the trades in this batch. If an asset's
    ///          price is missing or fails verification in the shared proof, its trade execution
    ///          will be skipped silently via the internal try/catch.
    function batchExecute(
        ...
        for (uint256 i = 0; i < len; i++) {
            uint256 supraId = riskProofs[i].supraId;
            uint256 oraclePrice;
            try this.getPriceFromInfoExternal(supraId, info) returns (uint256 p) {
                oraclePrice = p;
            } catch {
    ...
    function _executeTriggered(
        ...
        Trade storage t = trades[tradeId];
        if (t.supraId != rp.supraId) return false;

        AssetConfig storage cfg = assets[rp.supraId];
    ...
    {
        Trade storage t    = trades[tradeId];
        AssetConfig storage cfg = assets[t.supraId];
        ...
        uint256 newLong  = openInterestLong[t.supraId]  + (t.direction == DIR_LONG  ? oi : 0);
        uint256 newShort = openInterestShort[t.supraId] + (t.direction == DIR_SHORT ? oi : 0);
        ...
        if (traderOpenInterest[t.supraId][t.trader] + oi > cfg.maxTraderOI) return false;
        ...
        uint256 newTotalLockedCapital = _getNewTotalLocked(t.supraId, longDelta, shortDelta);
        ...
        _updateExposure(t.supraId, longDelta, shortDelta, newTotalLockedCapital);
        traderOpenInterest[t.supraId][t.trader] += oi;
    ...
    // INTERNAL — Close trade (all reasons share this path)
      function _closeTrade(
        ...
        if (reason == REASON_SL && t.guaranteedSL) {
            closePrice = t.stopLoss;
            gSlFee = (oi * cfg.guaranteedSLFeeBps) / PRECISION;
        } else {
        ...
        // 5. PnL Net = PnL Brut - Frais de Borrow - Fee Guaranteed SL
        int256 netPnl = rawPnl - int256(borrowFee) - int256(gSlFee);
        ...
        if (lossAmt >= (t.margin * cfg.liqThresholdBps) / PRECISION && !(reason == REASON_SL && t.guaranteedSL)) {
            reason = REASON_LIQ;
        }

        t.state          = reason == REASON_LIQ ? STATE_LIQUIDATED : STATE_CLOSED;
        t.closePrice     = closePrice;   // WITH spread pour market/TP/LIQ, RAW (prix garanti) pour SL garanti
        t.closeTimestamp = block.timestamp;
    ...
    function _pullFundsAndCommission(uint256 supraId, uint256 collateral, uint256 leverage)
    ...
    function _applyRisk(
        ...
        uint256 newLong  = openInterestLong[supraId]  + (direction == DIR_LONG  ? oi : 0);
        uint256 newShort = openInterestShort[supraId] + (direction == DIR_SHORT ? oi : 0);
        ...
        uint256 totalTraderOI = traderOpenInterest[supraId][msg.sender] + oi;
        ...
        uint256 newTotalLockedCapital = _getNewTotalLocked(supraId, longDelta, shortDelta);
        if (newTotalLockedCapital > vaultBalance) revert InsufficientVaultCapital();

        _updateExposure(supraId, longDelta, shortDelta, newTotalLockedCapital);

        traderOpenInterest[supraId][msg.sender] = totalTraderOI;
    }
    ...
    function _getNewTotalLocked(
        ...
        AssetConfig storage cfg = assets[supraId];
        uint256 oldLong = openInterestLong[supraId];
        uint256 oldShort = openInterestShort[supraId];
    ...
    function _updateExposure(
        ...
        AssetConfig memory cfg = assets[supraId];
        if (!cfg.listed) revert BadParameter();

        if (longDelta > 0) {
            openInterestLong[supraId] += uint256(longDelta);
        } else if (longDelta < 0) {
            uint256 sub = uint256(-longDelta);
            openInterestLong[supraId] = openInterestLong[supraId] > sub ? openInterestLong[supraId] - sub : 0;
        }

        if (shortDelta > 0) {
            openInterestShort[supraId] += uint256(shortDelta);
        } else if (shortDelta < 0) {
            uint256 sub = uint256(-shortDelta);
            openInterestShort[supraId] = openInterestShort[supraId] > sub ? openInterestShort[supraId] - sub : 0;
        }
    ...
    function _releaseExposure(uint256 tradeId) internal {
        ...
        traderOpenInterest[t.supraId][t.trader] = traderOpenInterest[t.supraId][t.trader] > oi
            ? traderOpenInterest[t.supraId][t.trader] - oi : 0;
    }
    ...
    function _storeTrade(TradeInit memory init) internal returns (uint256 tradeId) {
        ...
        t.guaranteedSL  = init.guaranteedSL;
    }
    ...
    // =========================================================
    // HELPERS — Oracle price (Supra timestamps are in milliseconds)
    // =========================================================

    function _getPrice(uint256 expectedSupraId, bytes calldata proof) internal returns (uint256) {
        ISupraOraclePull.PriceInfo memory info = oracle.verifyOracleProofV2(proof);
        return _getPriceFromInfo(expectedSupraId, info);
    }

    function _getPriceFromInfo(
        uint256 expectedSupraId,
        ISupraOraclePull.PriceInfo memory info
    ) internal view returns (uint256) {
        AssetConfig storage cfg = assets[expectedSupraId];
        if (!cfg.listed) revert BadParameter();

        for (uint256 i = 0; i < info.pairs.length; i++) {
            if (info.pairs[i] != expectedSupraId) continue;

            // Supra timestamps are in milliseconds — convert to seconds before comparing
            uint256 tsSeconds = info.timestamp[i] / 1000;
            if (tsSeconds > block.timestamp) {
                // Clock drift: allow a future timestamp as long as the drift is not too large
                if (tsSeconds - block.timestamp > cfg.maxProofAge) revert StalePrice();
            } else {
                if (block.timestamp - tsSeconds > cfg.maxProofAge) revert StalePrice();
            }
    ...
    function getPriceFromInfoExternal(
        uint256 expectedSupraId,
        ISupraOraclePull.PriceInfo calldata info
    ...
    function _verifyKmsProof(RiskProof calldata rp) internal view {
        ...
        bytes32 hash    = keccak256(abi.encode(
            rp.supraId, rp.maxOILong, rp.maxOIShort, rp.spreadLong, rp.spreadShort, rp.timestamp
        ));
    ...
    function _validateConfig(AssetConfig memory cfg) internal pure {
        ...
        if (cfg.guaranteedSLFeeBps > 100_000)   revert BadParameter();
    }
